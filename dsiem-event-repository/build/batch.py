...
LOGSTASH_JSON_HOST_DIR = os.getenv("LOGSTASH_JSON_HOST_DIR")
...
def load_plugin_presets(path="plugin_presets.json"):
    ...
    try:
        ...
        print("[WARN] Gagal memuat plugin_presets.json: {}. Fallback ke input manual.".format(e))
        return None
        ...
        with f:
            ...
            for r in rows:
                ...
                if isinstance(category, str):
                    try: cat = category.decode("utf-8")
                    except Exception: cat = category.decode("latin-1", "ignore")
                else:
                    cat = unicode(category or "")
                    
                if isinstance(kingdom, str):
                    try: king = kingdom.decode("utf-8")
...
def render_tsv(rows, siem_plugin_type, plugin_id, category, kingdom):
    ...
    try:
        ...
        if isinstance(category, str):
            ...
            cat_u = unicode(category or u"")
            
        if isinstance(kingdom, str):
            try: king_u = kingdom.decode("utf-8")
            except Exception: king_u = kingdom.decode("latin-1", "ignore")
        else:
            king_u = unicode(kingdom or u"")
...
def append_or_create_directive(meta_path, cfg_dir, registry, use_remote_defaults=False, out_filename=None, preset_answers=None):
    ...
    print(u"[PRESET/CFG] HEADER: {}".format(HEADER))
    ...
    try:
        ...
        if isinstance(HEADER, str):
            ...
            except Exception: HEADER = HEADER.decode("latin-1", "ignore")
        
        if isinstance(CATEGORY, str):
            try: CATEGORY = CATEGORY.decode("utf-8")
            except Exception: CATEGORY = CATEGORY.decode("latin-1", "ignore")

        if isinstance(KINGDOM, str):
            try: KINGDOM = KINGDOM.decode("utf-8")
...
def process_preset(preset_name, preset_config, auth):
    ...
    if not device_name: 
        print(u"[ERROR] 'device_name' tidak ada di preset '{}'.".format(preset_name))
        return

    default_index = "{}*".format(log_type_auto)
    index_pattern = preset_creator.get("index_pattern") or default_index
    if not index_pattern.endswith("*"): 
        index_pattern += "*"
    
    field_name = preset_creator.get("field_name") or "event_name"
    size_str = str(preset_creator.get("size", "100"))
    try:
        size = int(size_str)
    except ValueError:
        size = 100
    ...
    print("\n[INFO] terms agg field input : {}".format(field_name))
    print("\n[INFO] index pattern (untuk query & file 70) : {}".format(index_pattern))
    ...
    try:
        ...
        if not query_success:
            ...
            if alt_field == field_name and alt_filters == filters:
                if r: raise Exception(explain_http_error(r))
                else: raise Exception("[QUERY ERROR] Gagal pada percobaan pertama dan tidak bisa fallback.")
            else:
                ...
                try:
                    ...
                    if r2.status_code < 300:
                        ...
                        if data_check.get("aggregations",{}).get("event_names",{}).get("buckets",[]):
                            ...
                            raise Exception("[QUERY ERROR] Query fallback berhasil tapi tidak ada hasil. Cek field dan filter lagi.\n- Percobaan 1: {}\n- Percobaan 2: HTTP {}".format(explain_http_error(r) if r else "Request Error", r2.status_code))
                    else:
                        raise Exception("[FIELD/QUERY ERROR]\n- Percobaan 1: {}\n- Percobaan 2: {}".format(explain_http_error(r) if r else "Request Error", explain_http_error(r2)))
                except requests.exceptions.RequestException as e:
                    raise Exception("[HTTP ERROR] (saat retry .keyword) {}".format(e))
                except (ValueError, KeyError):
                    raise Exception("[PARSE ERROR] Response dari query fallback bukan JSON valid.")

        if not query_success:
            raise Exception("[FATAL] Gagal mendapatkan data event setelah mencoba semua fallback.")
        
        print("[INFO] Sukses menggunakan field: '{}'".format(field_name))

        if r.status_code==404: raise Exception("[INDEX ERROR] Index pattern '{}' tidak ditemukan.".format(index_pattern))
        if r.status_code>=300: raise Exception(explain_http_error(r))
        try: data=r.json()
        except ValueError: raise Exception("[PARSE ERROR] Response bukan JSON valid")

    except Exception as e:
        print(u"[WARN] Query Elasticsearch GAGAL: {}".format(e))
        print("[WARN] Proses build file akan tetap dilanjutkan menggunakan data TSV dari GitHub (jika ada).")

    buckets = data.get("aggregations",{}).get("event_names",{}).get("buckets",[])
    
    if not buckets:
        print("[INFO] Tidak ada bucket baru dari agregasi. Proses build file akan menggunakan data TSV dari GitHub (jika ada).")
    ...
    f1_val = filters[0]["value"] if filters and filters[0].get("value") else ""
    ...
    if existing_pid_for_spt is not None:
        ...
        pid_in = str(preset_plugin.get("plugin_id_new", "")).strip()
        if not pid_in.isdigit(): 
            print(u"[ERROR] 'plugin_id_new' ('{}') di preset bukan angka.".format(pid_in))
            return
        
        chosen_pid = int(pid_in)
        if chosen_pid in used_pids: 
            print(u"[ERROR] 'plugin_id_new' ({}) di preset sudah dipakai.".format(chosen_pid))
            return

        plugin_id_final = chosen_pid
        print(u"[PRESET] Akan menggunakan plugin_id BARU: {}".format(plugin_id_final))
    ...
    if config_file:
        try:
            ...
            print("[CFG] config.json (remote) ditemukan di GitHub.")
    ...
    directive_category = default_category
    directive_kingdom = default_kingdom
    ...
    print(u"Path template Logstash (auto-confirmed): {}".format(template_path_70))
    ...
    print(u"Path template Vector (auto-confirmed): {}".format(template_path_vector))
    ...
    if not ans2_preset or ans2_preset.lower() not in ("y", "n"):
        ...
        print(u"[PRESET] Generate directive: {}".format(ans2_preset))
        ans2 = ans2_preset
    ...
    config_payload = { 
        "module": module_slug, 
        "submodule": submodule_slug, 
        "file70": conf_meta.get("file70_cfg", {}), 
        "vector": conf_meta.get("vector_cfg", {}), 
        "directive": directive_cfg_out or directive_cfg_default, 
        "mappings": field_data 
    }
...
def main():
    ...
    print(u"\n[INFO] Ditemukan {} preset. Memulai proses batch...".format(len(presets)))
    ...
    print(u"\n=======================================================")
    print(u"=== PROSES BATCH SELESAI ===")
    print(u"- Sukses: {}".format(success_count))
    print(u"- Gagal : {}".format(fail_count))
    print(u"=======================================================")
